...
<details>
  ...
  <p>You can subscribe to this group by <a href="{{ group.url }}" class="button" title="Open the Group for the {{ group.name }} (Opens in a new window)" target="_blank">visiting the Group page</a> or by sending an email to:
  <br><code>{{ group.email | replace: "@", "+subscribe@" }}</code></p>
